        ...
        <form id="form1" name="form1" method="post" action="">
                            ...
                            <table>
                                <tr>
                                    <td><label for="txt_f">From Date</label>
                                        <input type="date" name="txt_f" id="txt_f" max="<?php echo date('Y-m-d')?>" class="form-control">
                                    </td>

                                    <td><label for="txt_t">To Date</label>
                                        <input type="date" name="txt_t" id="txt_t" max="<?php echo date('Y-m-d')?>" class="form-control">
                                    </td>
                                </tr>
                                <tr>
                                    <td>
            ...
            if (isset($_POST["btnsave"])) {
                ...
                while ($data = $row->fetch_assoc()) {
                    ...
                    $sel1 = "select IFNULL(sum(cart_quantity), 0) as id from tbl_cart c inner join tbl_product p on c.product_id=p.product_id inner join tbl_booking o on c.booking_id=o.booking_id where ( cart_status in (1,3,4) and p.seller_id = '" . $data["seller_id"] . "' and o.booking_date between '" . $_POST["txt_f"] . "' and '" . $_POST["txt_t"] . "')";
                    $row1 = $con->query($sel1);
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            $sel = " select * from tbl_booking o inner join tbl_cart c on o.booking_id=c.booking_id inner join tbl_product p on c.product_id=p.product_id inner join tbl_seller s on s.seller_id=p.seller_id where ( cart_status in (1,3,4) and o.booking_date between '" . $_POST["txt_f"] . "' and '" . $_POST["txt_t"] . "')";
                                            $row = $con->query($sel);
                                            $i = 0;
                                            while ($data = $row->fetch_assoc()) {
                                                $i++;
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php echo $data["seller_contact"]; ?><br><?php echo $data["seller_email"]; ?>
                                                    </td>
                                                    <td>
                ...
                while ($data = $row->fetch_assoc()) {
                    $xValues[] = $data["seller_name"];
                    $sel1 = "select IFNULL(sum(cart_quantity), 0) as id from tbl_cart c inner join tbl_product p on c.product_id=p.product_id inner join tbl_booking o on c.booking_id=o.booking_id where ( cart_status in (1,3,4) and p.seller_id = '" . $data["seller_id"] . "')";
                    $row1 = $con->query($sel1);
                    while ($data1 = $row1->fetch_assoc()) {
                        $yValues[] = $data1["id"];
                    }
